...
Too high=above three months of average sales
...
---Query 10: Flagging safety stock and reorder point as too high,too low or appropriate
---LOGIC:To compare against average monthly sales whch serves as the benchmark
---NOTE: See assumptions in header for threshold definitions
...
s.AvgMonthlySales,
CASE
WHEN p.SafetyStockLevel < s.AvgMonthlySales
THEN 'Too Low'
WHEN p.SafetyStockLevel > s.AvgMonthlySales * 3
THEN 'Too High'
ELSE 'Appropriate'
END AS SafetyStockStatus,
CASE
WHEN p.ReorderPoint < s.AvgMonthlySales
THEN 'Too Low'
WHEN p.ReorderPoint > s.AvgMonthlySales * 3
THEN 'Too High'
ELSE 'Appropriate'
END AS ReorderPointStatus
FROM Production.Product p
INNER JOIN (
SELECT
s.ProductID,
SUM(s.OrderQty) AS TotalSales,
SUM(s.OrderQty) / NULLIF(COUNT(DISTINCT MONTH(so.OrderDate)), 0) AS AvgMonthlySales
FROM Sales.SalesOrderDetail s
...
/*
Query 11: Summarising total inventory and total sales by category
...
/*
Query 12: Identifies products flagged as non-salable that still exist in inventory.
...
/*
Query 13: Identifies products that were once salable but have exceeded their SellEndDate and still have inventory remaining.
...
---Query 14: Products with the least stock coverage(closest to running out of stock relative to how fast they sell)
...
NULLIF(s.AvgMonthlySales, 0) AS MonthsOfStockRemaining
FROM (
SELECT ProductID, SUM(Quantity) AS TotalQuantity
FROM Production.ProductInventory
GROUP BY ProductID
